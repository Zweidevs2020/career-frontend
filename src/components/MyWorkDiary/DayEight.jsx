...
const DayEight = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const isEmpty =
      !formData.date ||
      !formData.jobDemands ||
      !formData.workImportance ||
      !formData.enjoyablePart ||
      !formData.worstPart ||
      !formData.technologyChange ||
      !formData.tradeUnion ||
      !formData.industrialProblem ||
      !formData.trainingOpportunities ||
      !formData.promotionEase ||
      !formData.careerOpportunities ||
      !formData.recommendedTraining ||
      !formData.specificSkills.length ||
      formData.jobs?.every((job) => !job); // Ensure at least one job is filled

    // Check if "Has..." fields require explanations
    const hasIndustrialProblem =
      formData.industrialProblem === "Yes" &&
      !formData.industrialProblemExplanation;
    const hasTrainingOpportunities =
      formData.trainingOpportunities === "Yes" &&
      !formData.trainingOpportunitiesExplanation;

    if (isEmpty || hasIndustrialProblem || hasTrainingOpportunities) {
    ...
    const questionsAndAnswers = [
      ...
      formData.trainingOpportunities === "Yes" && {
        question: "If yes, what type of training opportunities?",
        answer: formData.trainingOpportunitiesExplanation,
      },
      {
        question: "Can you get promotion easily?",
        answer: formData.promotionEase,
    ...
    if (!formData.date) {
      ...
      try {
        const url = `${API_URL.WORK_DIARY}update-day/?day=Day8`; // Adjust day dynamically as needed
