...
const DayTen = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const isEmpty =
      !formData.date ||
      !formData.selfLearning ||
      !formData.demonstratedSkills ||
      !formData.goalsAchieved ||
      (formData.goalsAchieved === "No" && !formData.goalsAchievedExplanation) || // Check if "Why" is required when "No"
      !formData.worstExperience ||
      !formData.bestExperience ||
      !formData.performanceRatings ||
      !formData.supervisorComparison ||
      (formData.supervisorComparison === "No" &&
        !formData.comparisonDifference); // Check if "Why" is required when "No"
    ...
    // Check if any questions and answers are empty
    if (questionsAndAnswers.some((q) => !q.answer)) {
      message.error("All fields must be filled out.");
      setLoading(false);
    ...
    // Post data if fields are filled or update data if it exists
    try {
      if (isEmpty) {
        // Post data if any required field is missing
        message.error("Please fill in all required fields!");
        return; // Exit the function to prevent submission
      } else {
        const apiUrl = API_URL.WORK_DIARY;
        await postApiWithoutAuth(apiUrl, payload); // Post the data if no errors
      }
    } catch (error) {
    ...
    // Check for updating existing data
    try {
      if (!isEmpty) {
