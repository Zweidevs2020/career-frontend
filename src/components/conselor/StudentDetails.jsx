...
const ConselorWrorkDiay = () => {
  ...
  // --- 4. Sort the days in natural order (e.g., Day 1, Day 2, etc.) ---
  ...
  return (
    <div className="p-6">
      ...
      {/* 5. Renamed heading to WORK DIARY */}
      <h1 className="text-2xl font-bold mb-4"> My Work Diary</h1>

      {/* 6. Display the data or loading/error states */}
      {loading ? (
        <p>Loading...</p>
      ) : workExperience.length > 0 ? (
        // 7. Render each "day" in its own card, sorted in natural order
        sortedDayKeys.map((dayKey, index) => {
          // Format the dayKey to add a space if missing (e.g., "Day3" -> "Day 3")
          const formattedDayKey = dayKey.replace(/(Day|Da)(\d+)/, "$1 $2");
          return (
            <div
              key={index}
              className="fade-in bg-white shadow-lg rounded-lg p-6 mb-4"
                  ...
                  <p className="text-gray-600">
                    ...
                    {(() => {
                      try {
                        ...
                        return (
                          <ul className="list-disc pl-5">
                            {Object.entries(parsedAnswer).map(
                              ([key, value]) => (
                                <li key={key}>
                                  <strong>{key}:</strong> {value}
                                </li>
...
export default ConselorWrorkDiay;
